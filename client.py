...
class client:
    ...
    def sync_message(self, str):
        ...
        return [0, self.an, self.bn, hash_function([self.K, self.id, self.c, self.an, self.bn, self.n])]

    def desync_message(self, str):
        rn = get_random()
        yn = self.an ^ self.id ^ rn
        zn = hash_function([self.K, rn, yn])
        return [1, self.an, self.bn, yn, zn, hash_function([self.K, self.id, self.c, self.an, self.bn, self.n, zn])]
